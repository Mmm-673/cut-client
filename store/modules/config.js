...
function applyTabBarLabel(effectiveReviewMode) {
  try {
    // 微信限制：当前页面不是 tabBar 页时调用会报 not TabBar page，直接跳过
    const pages = typeof getCurrentPages === 'function' ? getCurrentPages() : []
    const currentRoute = pages.length ? pages[pages.length - 1].route : ''
    if (!TAB_PAGE_ROUTES.includes(currentRoute)) return
    uni.setTabBarItem({
      index: 1,
      text: effectiveReviewMode ? '球厅' : '预约',
      fail: () => {
        // 忽略偶发失败，进入 tab 页时 syncTabBarLabel 会重试
      }
    })
  } catch (e) {
...
export const useConfigStore = defineStore('config', () => {
  ...
  // 供 tab 页面 onShow 调用，确保 tabBar 文案与当前模式一致
  const syncTabBarLabel = () => {
    applyTabBarLabel(finalReviewMode.value)
  }
