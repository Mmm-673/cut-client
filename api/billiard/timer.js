 ...
 * @returns {number} returns.data.plannedDuration - 预定总时长（分钟，含加钟）；固定价订单为 0
 * @returns {number} returns.data.addedDuration - 加钟累计时长（分钟）；固定价订单为 0
 * @returns {number|null} returns.data.remainingSeconds - 剩余秒数；固定价订单为 null（不计算剩余）
